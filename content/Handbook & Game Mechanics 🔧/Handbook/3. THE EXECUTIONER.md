...
Here’s a quick overview of what’s on the executioner sheet. You can find a link to the printable version of the character sheet here (**NEEDS LINKS**), so grab it and follow along - it will help when going through the next section. 

Agendas and Defiance are kept on their own separate page. You can find it with the above sheet. 
...
2. Execution Tag (aka Damage Tag)
3. Skills 
...
These powers are created by a high concentration of **grace**, an unknown force that causes a grace seed, a tiny tumor like growth in the shape of a tiny sprout-like figure to manifest in the brain (or rarely, the heart), theorized to be the source of these powers.  

**MENTAL**

An executioner’s general skill with their psychic powers is rated by the special MENTAL skill. This is a special skill equal to half your total category, rounded up (so a CAT 4 executioner would have 2 MENTAL). It does not increase any other way, though you can still gain advantage dice on it through aid, setup, etc.
...
• You can always spend a psyche burst to add +1 advantage die to any roll (yours or an ally’s) by using your defiance in a creative way.  
...
If an executioner would exceed their **strain overflow limit**, which is normally 10, they go into **strain overflow** at the end of the scene. They lose control of the psychic energies in their body, which start to run wild and begin their transformation into **an anomaly**. A character in Strain Overflow has two choices: **give up** or **resist**.
...
They make one final choice, which is to **flee** or **fight**. Fighting immediately forces a conflict scene with an Imago. The new Anomaly may become a target of a later hunt, or a greater antagonist.
